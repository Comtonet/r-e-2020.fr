...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
    ...
    if($cfg['base_url']===''||$cfg['token']===''){
        ...
        curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_CONNECTTIMEOUT=>4,CURLOPT_TIMEOUT=>8,CURLOPT_HTTPHEADER=>['Authorization: Bearer '.$cfg['token'],'Accept: application/json']]);
        ...
        else $result=['ok'=>!empty($json['ok'])&&$status>=200&&$status<300,'title'=>!empty($json['ok'])?'Liaison opérationnelle':'Liaison refusée','message'=>(string)($json['message']??('HTTP '.$status)),'status'=>$status];
    }
}
$cfg=keepote_bridge_settings();
$pendingFile=sys_get_temp_dir().'/keepote-history-pending.jsonl';
$pending=0;
